...
export function buildPromoCard(product = {}, opts = {}) {
  ...
  if (price != null) {
    ...
    const unit = product.unitText ? `${product.unitText}ละ` : 'ราคาพิเศษ';
    // Optional struck normal price (small grey) when there is a separate sale price.
    if (Number.isFinite(product.priceNormal) && Number.isFinite(product.priceSale) && product.priceSale < product.priceNormal) {
      els.push({
        type: 'text', text: unit + ' ฿' + money(product.priceNormal), x: bx + bw / 2, y: by + bh * 0.22,
        size: Math.round(bh * 0.13), color: '#888888', weight: 'normal', align: 'center', strike: true, maxWidth: bw - 20,
      });
    } else {
      els.push({
        type: 'text', text: unit, x: bx + bw / 2, y: by + bh * 0.23,
        size: Math.round(bh * 0.15), color: INK, weight: 'bold', align: 'center', maxWidth: bw - 20,
      });
    }
    els.push({
      type: 'text', text: money(price) + '.-', x: bx + bw / 2, y: by + bh * 0.60,
      size: Math.round(bh * 0.42), color: RED, weight: 'bold', align: 'center', maxWidth: bw - 14,
    });
  }
